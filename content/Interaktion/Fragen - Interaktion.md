> [!info]
> Fragen unterteilt in Lano Teil und Weiß Teil
> [zum Lano Teil](#Lano)
> [zum Weiß Teil](#Weiß)
> - Fragen basieren auf Vorlesung zur Prüfungsvorbereitung
# Lano
## Basics
...
Bewegendes Objekt, wie ein Auto, vom Hintergrund unterscheiden.
	-> dafür machen wir erst ein Referenzbild vom leeren Hintergrund
	-> dann vergleicht man Pixel des aktuellen Bildes mit denen des Referenzbildes
	-> Unterschiede zeigen die Position des Objekts

<u>Schritte</u>:
1. Leeres Bild aufnehmen:
	- Bild ohne Auto als "Hintergrundbild" speichern.
2. Pixel vergleichen:
	- für jeden Pixel wird der Unterschied zwischen dem aktuellen Bild und Hintergrund berechnet.
3. Thresholding:
	- Alles was unter der Grenze wird "schwarz", alles was Hell ist, wird "weiß" markiert -> binarisiertes Bild.
4. Konturen finden:
	- mit `cv2.findContours` werden Umrisse erkannt.
5. Sortieren:
	- Um Störungen zu vermeiden, wählt man nur die größten Umrisse aus.

<u>Infrarot</u>
- Bei einem Szenario, in dem ein Auto auf einem Tisch fährt und von einem Beamer beleuchtet wird, hilft Infrarot, das Beamerlicht auszublenden.

- Infrarotfilter vor der Kamera lässt nur Infrarotlicht durch und blockiert sichtbares Licht
	-> stört das Beamerlicht nicht.
- IR-Beleuchtung nötig -> damit die Kamera etwas "sehen" kann, braucht es eine separate Infrarotbeleuchtung.

Benötigte Komponenten:
- **Kamera**: Am besten SW -> da besonders empfindlich für Infrarot
- **IR-Filter**: Blockiert sichtbares Licht und lässt nur Infrarot durch.
- **IR-Beleuchtung**: Beleuchtet Szene im Infrarotbereich
- **Computer & Kabel**

## Sensoren
- wandeln physikalische Effekte in elektrische Signale um.

<u>Eigenschaften</u>
- **Empfindlichkeit**: Wie genau erkennt Sensor kleine Änderungen.
- **Genauigkeit**: Wie präzise sind die Messwerte.
- **Störungsunabhängigkeit**: Wie gut arbeitet Sensor bei externen Einflüssen.
- **Linearität**

<u>Typen</u>
- LIDAR (Light Detection and Ranging)
	- Abstandsmessung und Umgebungsmodellierung
	- sehr präzise Distanzmessung
- Ultrasonic-Sensoren
	- Abstandsmessung auf kurze Distanz (z.B. Einparkhilfen)
	- gut für kleine Abstände
- Infrarotsensoren
	- Hinderniserkennung und Tracking (z.B. in Kombination mit Lichtschranken oder Kameras)
- Radarsensoren
	- Abstands- und Geschwindigkeitsmessung (z.B. Tempomat, Spurhalteassistent)
- Laser
	- Präzise -> Entfernungen oder Positionen sehr genau
	- sowohl auf kurze als auch lange Distanz einsetzbar
	- sehr schnell
- Gyroskop
	- misst Rotationsgeschwindigkeit
- Beschleunigungssensor
	- misst lineare Beschleunigung durch Erkennung der Schwerkraft
- Magnetometer
	- misst Magnetfeld, um absolute Orientierung zu bestimmen.
